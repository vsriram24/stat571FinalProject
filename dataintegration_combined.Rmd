...
newHappiness_filtered_imputed$NursingMidwife[is.na(newHappiness_filtered_imputed$NursingMidwife)] <- mean(newHappiness_filtered_imputed$NursingMidwife, na.rm = TRUE)

newHappiness_filtered_imputed$PharmacistsPer10000[is.na(newHappiness_filtered_imputed$PharmacistsPer10000)] <- mean(newHappiness_filtered_imputed$PharmacistsPer10000, na.rm = TRUE)

newHappiness_filtered_imputed$PopHealthcareExpGreaterThan10[is.na(newHappiness_filtered_imputed$PopHealthcareExpGreaterThan10)] <- mean(newHappiness_filtered_imputed$PopHealthcareExpGreaterThan10, na.rm = TRUE)

newHappiness_filtered_imputed$PopHealthcareExpGreaterThan25[is.na(newHappiness_filtered_imputed$PopHealthcareExpGreaterThan25)] <- mean(newHappiness_filtered_imputed$PopHealthcareExpGreaterThan25, na.rm = TRUE)

newHappiness_filtered_imputed$RoadTrafficDeathRatePer100000[is.na(newHappiness_filtered_imputed$RoadTrafficDeathRatePer100000)] <- mean(newHappiness_filtered_imputed$RoadTrafficDeathRatePer100000, na.rm = TRUE)

newHappiness_filtered_imputed$tobaccoAge15plus[is.na(newHappiness_filtered_imputed$tobaccoAge15plus)] <- mean(newHappiness_filtered_imputed$tobaccoAge15plus, na.rm = TRUE)

newHappiness_filtered_imputed$under5MortalityRate[is.na(newHappiness_filtered_imputed$under5MortalityRate)] <- mean(newHappiness_filtered_imputed$under5MortalityRate, na.rm = TRUE)

newHappiness_filtered_imputed = newHappiness_filtered_imputed %>%
  select(-X, -year)
```

# Investigate Variable Correlation
```{r}
varcorr <- cor(newHappiness_filtered_imputed[-1])
library(corrplot)
corrplot(varcorr, type = "upper", order = "hclust", 
...
x_newHappiness = newHappiness_filtered_imputed %>%
  select(-Location, -Happiness.Score, -NTDIntervention)
...
```{r}
summary(newHappiness.centered.pca)$loading
...
Explain taking out variables and countries with NAs.
...
We can see from this plot that lambda.1se is ~0.12 and has ~8 nonzero coefficients.
...
summary(fit.1se.lm)
```
The model we get from Lasso includes 8 variables, although not all of them are significant at the .05 level. Some additional tuning of the model is required.
...
plot(summary(fit.exh)$cp)
```

We see from the cp plot that six variables minimizes cp.

```{r select variables cp}
...
All variables are significant at the .05 level, so we don't need to eliminate further variables with backwards selection.
...
From the residual and normal Q-Q plot, we can see that the linear model assumptions are generally satisfied.
