...
```{r}
set.seed(123)
newHappiness.kmeans <- as.data.frame(x_newHappiness) %>% kmeans(centers = 2 ,nstart = 1)
...
data.frame(pc1 = newHappiness.centered.pca$x[, 1],
  ...
  ggplot(aes(x = pc1, y = pc2, col = group)) + geom_point() + ggtitle("Clustering over PC1 and PC2")
```
